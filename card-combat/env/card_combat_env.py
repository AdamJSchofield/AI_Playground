...
class raw_env(AECEnv):
    ...
    def __init__(self, render_mode=None):
        ...
        self._observation_spaces = {
            agent: Dict(spaces={
                "observation": MultiDiscrete(self.game.OBSERVATION_SHAPE, dtype=np.int8),
    ...
    def state(self, agent):
        return self.state[agent](agent)
    
    def observe(self, agent):
        return self.observations[agent](agent)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = {agent: lambda agent: self.game.get_player_state(agent) for agent in self.agents}
        self.observations = {agent: lambda agent: self.game.get_player_state(agent) for agent in self.agents}
        self.num_moves = 0
        self.num_rounds = 1
    ...
    def step(self, action):
        ...
        if self.is_first_turn:
            if self._agent_selector.is_first():
                self.render_in_step(f'\nROUND: {self.num_rounds}')
            self.is_first_turn = False
            self.render_in_step(f'HEALTH: \t{self.agents[0]}: {self.game.players[self.agents[0]].health}\t{self.game.players[self.agents[1]].health} :{self.agents[1]}')
            self.render_in_step(f'{agent} HAND: {str([card.cardType.name for card in self.game.players[agent].hand])}')
        #self.render_in_step(f'\tDEBUG: "{agent}: {str(self.observations[agent](agent))}\n\t\t{other_agent}: {str(self.observations[other_agent](other_agent))}')
