...
if __name__ == "__main__":
    ...
    # Train against random from both sides
    print("TRAINING: Player" + str(args.agent_id) + " OPPONENT: " + "Random" if args.opponent_path == '' else "Trained")
    result, agent = train_agent(args)
    watch(args, agent)

    args.agent_id = 1 - args.agent_id
    print("TRAINING: Player" + str(args.agent_id) + " OPPONENT: " + "Random" if args.opponent_path == '' else "Trained")
    result, agent = train_agent(args)
    watch(args, agent)

    args.resume_path = 'log/card_combat/pg/policy.pth'
    args.opponent_path = 'log/card_combat/pg/policy.pth'

    print("TRAINING: Player" + str(args.agent_id) + " OPPONENT: " + "Random" if args.opponent_path == '' else "Trained")
    result, agent = train_agent(args)
    result, agent = train_agent(args, agent_opponent=agent)
    watch(args, agent)
