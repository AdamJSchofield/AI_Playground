...
def get_pg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--training-envs', type=int, default=10, help='number of parallel training environments')
    parser.add_argument('--test_envs', type=int, default=10, help='number of parallel testing environments')
    parser.add_argument('--win-rate', type=float, default=0.95, help='win rate (if win reward is 1) or mean reward to stop training')
    parser.add_argument('--agent-id', type=int, default=0, help='the learned agent plays as the agent_id-th player. Choices are 0 and 1')
    
    # log/card_combat/pg/policy.pth
    parser.add_argument('--resume-path', type=str, default='log/card_combat/pg/policy.pth', help='the path of agent pth file for resuming from a pre-trained agent')
    parser.add_argument('--opponent-path', type=str, default='log/card_combat/pg/policy.pth', help='the path of opponent agent pth file for resuming from a pre-trained agent')
