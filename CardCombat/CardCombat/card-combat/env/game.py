...
class game:
    ...
    def __init__(self, player_names, player_healths = [50, 50], player_energies = [3, 3], player_decks= [DECK_TYPE.STANDARD, DECK_TYPE.STANDARD]):
        ...
        self.OBSERVATION_SHAPE = flatten_observation({
            ...
            "player_energy": max(player_energies) + 1,
    ...
    def get_player_state(self, player_name):
        ...
        observation = {
            ...
            "cards_played": pad_list(player.cards_played, self.HAND_SIZE, len(ALL_CARDS)), # If ALL_CARDS contains types [0, 1, 2], pad with 3's for no action
            "opponent_cards_played": pad_list(other_player.cards_played, self.HAND_SIZE, len(ALL_CARDS))
        }
